...
import { Plus, Trophy, ArrowLeft, Swords } from "lucide-react";
...
export function SessionDetail() {
  ...
  const { mutate: createMatch, isPending } = useCreateMatch({
    mutation: {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: getGetPlayersQueryKey() }); // update balances globally
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMatch({
      data: {
        sessionId,
        team1Player1Id: parseInt(t1p1),
        team1Player2Id: parseInt(t1p2),
        team2Player1Id: parseInt(t2p1),
        team2Player2Id: parseInt(t2p2),
        winnerTeam: winner,
        betAmount
      }
    });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border pb-6">
        <div>
          ...
          {session.guestPlayerName && (
            <p className="text-slate-400 mt-1">Guest: {session.guestPlayerName}</p>
          )}
        </div>
        <button 
              ...
              >
                ...
                <div className="flex items-stretch justify-between gap-4 relative z-10">
                  ...
                  <div className={cn(
                    ...
                    match.winnerTeam === 1 ? "border-primary bg-primary/5 shadow-[0_0_15px_rgba(16,185,129,0.1)]" : "border-slate-800 bg-slate-900/50"
                  )}>
                  ...
                  <div className="flex items-center justify-center text-slate-500 font-display font-bold italic pt-4">
                    VS
                  </div>
                  ...
                  <div className={cn(
                    "flex-1 p-4 rounded-xl border-2 text-center transition-colors",
                    match.winnerTeam === 2 ? "border-primary bg-primary/5 shadow-[0_0_15px_rgba(16,185,129,0.1)]" : "border-slate-800 bg-slate-900/50"
                  )}>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            ...
            <div className="hidden md:flex absolute inset-0 items-center justify-center pointer-events-none">
              <div className="w-10 h-10 bg-slate-950 border border-slate-800 rounded-full flex items-center justify-center text-slate-500 font-bold z-10">
                VS
              </div>
            </div>
            ...
            {/* Team 1 Setup */}
            <div className={cn(
              "space-y-4 p-5 rounded-2xl border-2 transition-all",
              winner === 1 ? "border-primary bg-primary/5" : "border-slate-800 bg-slate-900/30"
            )}>
            ...
            <div className={cn(
              "space-y-4 p-5 rounded-2xl border-2 transition-all",
              winner === 2 ? "border-primary bg-primary/5" : "border-slate-800 bg-slate-900/30"
            )}>
              <div className="flex justify-between items-center mb-2">
          ...
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <input 
                ...
                className="w-24 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white font-bold text-center focus:outline-none focus:border-primary"
              />
            </div>
          </div>
          ...
          <button 
            type="submit" 
            disabled={isPending || !winner || !t1p1 || !t1p2 || !t2p1 || !t2p2}
