    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <h1 className="text-4xl sm:text-5xl font-display font-extrabold text-white mb-2">
              Squad <span className="text-primary">Balances</span>
            </h1>
            <p className="text-slate-400 text-lg">Track bets, court fees, and group debts.</p>
          </div>
          <button 
          ...
          players?.map((player, index) => (
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <div className={cn(
                  "p-3 rounded-xl",
                  player.balance > 0 ? "bg-emerald-500/10 text-emerald-500" : 
                  player.balance < 0 ? "bg-red-500/10 text-red-500" : 
              ...
              <div className="flex items-end justify-between">
                <div className="flex items-center gap-2">
                  <span className={cn(
                    ...
                    player.balance > 0 ? "text-emerald-400" : 
                    player.balance < 0 ? "text-red-400" : 
                    "text-white"
                  )}>
                    {formatCurrency(player.balance)}
                  </span>
                  {player.balance > 0 && <TrendingUp size={20} className="text-emerald-500 mb-1" />}
                  {player.balance < 0 && <TrendingDown size={20} className="text-red-500 mb-1" />}
                </div>
      ...
      <Modal isOpen={isAddPlayerModalOpen} onClose={() => setIsAddPlayerModalOpen(false)} title="Add New Player">
        <form onSubmit={handleAddPlayer} className="space-y-6">
          ...
          <label className="flex items-center gap-3 p-4 rounded-xl border border-slate-800 bg-slate-900/50 cursor-pointer hover:bg-slate-800 transition-colors">
            <input 
              type="checkbox" 
              name="isFixed"
              className="w-5 h-5 rounded border-slate-700 text-primary focus:ring-primary focus:ring-offset-slate-900 bg-slate-950" 
            />
            <div>
              <p className="font-medium text-white">Fixed Member</p>
              <p className="text-xs text-slate-400">Fixed members show up prominently</p>
            </div>
          </label>
          ...
          >
            {isCreatingPlayer ? <div className="w-6 h-6 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin" /> : "Add Player"}
          </button>
        </form>
      </Modal>

      {/* Top Up Modal */}
      <Modal isOpen={isTopUpModalOpen} onClose={() => setIsTopUpModalOpen(false)} title="Adjust Balance">
        <form onSubmit={handleTopUp} className="space-y-6">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-slate-500">Use negative numbers to deduct from balance.</p>
          </div>
          ...
          >
            {isUpdatingBalance ? <div className="w-6 h-6 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin" /> : "Confirm Transaction"}
          </button>
        </form>
      </Modal>
    </div>
